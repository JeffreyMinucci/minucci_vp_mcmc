...
#load field initial data
initial_conditions <- read.csv(vp_field_initials)
...
###   2) Write VP inputs
write_vp_input_sites(inputdata[1,], vpdir_in_control, initial_conditions)
...
for(i in 2:nsims){
  ...
  write_vp_input_sites(proposal_all, vpdir_in_control, initial_conditions)
