...
```{r}
test_results <- readRDS(file=paste(vpdir,"output/mcmc_run_objects/run_3_28_18_p5.rds",sep = ""))
print(paste("Acceptance rate:", test_results$accept_rate))
```
...
test_results <- continue_vp_mcmc(50000,test_results,step_length = .2)
saveRDS(test_results,file=paste(vpdir,"output/mcmc_run_objects/run_3_28_18_p8.rds",sep = ""))
test_results <- continue_vp_mcmc(50000,test_results,step_length = .2)
saveRDS(test_results,file=paste(vpdir,"output/mcmc_run_objects/run_3_28_18_p8.rds",sep = ""))
...
par(mfrow=c(1,1))
MCMCtrace(as.matrix(test_results$param_trace[, test_results$mcmc_params$optimize_vars[["names"]]]),filename="test_345k_thin1k",
          iter=345000,wd=paste(vpdir,"reports/figures/",sep="")) #creates pdf in reports/figures folder
...
raftery.diag(results_bt, q=0.5, r=0.1,s=0.95,converge.eps=0.001) #http://www.cryoung.org/wp-content/uploads/2014/09/MCMC-workshop-MBARI-2006.pdf


MCMCtrace(results_bt,filename="test_345k_thin1k",
          iter=345000,wd=paste(vpdir,"reports/figures/",sep="")) #creates pdf in reports/figures folder
