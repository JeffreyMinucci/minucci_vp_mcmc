...
system.time(test_results <- new_vp_mcmc_c(nsims = 10000, step_length = .3, vp_dir=vpdir, static_vars = static_list, 
                                          optimize_vars = optimize_list, logs = F, verbose = F, debug=F))[3]
#str(test_results)
print(paste("Acceptance rate:", test_results$accept_rate))
saveRDS(test_results,file=paste(vpdir,"output/mcmc_run_objects/run_5_2_18.rds",sep = ""))
...
```{r}
library(MCMCvis)
MCMCtrace(as.matrix(test_results$param_trace[, test_results$mcmc_params$optimize_vars[["names"]]]),filename="test2_10k_first9k",
          iter=9000,wd=paste(vpdir,"reports/figures/",sep=""))
```
