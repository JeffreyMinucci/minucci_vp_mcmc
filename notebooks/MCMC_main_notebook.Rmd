...
```{r}
test_results <- readRDS(file=paste(vpdir,"output/mcmc_run_objects/run_3_28_18.rds",sep = ""))
print(paste("Acceptance rate:", test_results$accept_rate))
```
...
test_results <- continue_vp_mcmc(30000,test_results,step_length = .3)
#saveRDS(test_results,file=paste(vpdir,"output/mcmc_run_objects/run_3_28_18_p4.rds",sep = ""))
...
par(mfrow=c(1,1))
MCMCtrace(as.matrix(test_results$param_trace[, test_results$mcmc_params$optimize_vars[["names"]]]),filename="test_65k_last60",
          iter=60000,wd=paste(vpdir,"reports/figures/",sep="")) #creates pdf in reports/figures folder
...
heidel.diag(results)
#last_20 <- mcmc(results[15000:35000,],start=15000)
heidel.diag(last_20)
```